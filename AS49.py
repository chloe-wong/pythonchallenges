...
def search_by_speed():
    cur = conn.execute('SELECT TopSpeed, Name FROM Cars WHERE TopSpeed > 210 ORDER BY TopSpeed",(MinSpeed,))
    cur2 = conn.execute("SELECT TopSpeed, Name FROM Cars WHERE TopSpeed < 200 ORDER BY TopSpeed DESC", (MinSpeed,))
    for row in cur:
